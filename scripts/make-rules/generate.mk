...
gen.add-copyright: tools.verify.license tools.verify.addlicense ## 添加版权头信息.
...
gen.ca: ## 生成 CA 文件.
	...
	@openssl req -new -nodes -key $(OUTPUT_DIR)/cert/ca.key -sha256 -days 3650 -out $(OUTPUT_DIR)/cert/ca.csr \
		-subj "/C=CN/ST=Guangdong/L=Guangzhou/O=srxstack/OU=it/CN=127.0.0.1/emailAddress=kalandramo@gmail.com"
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/server.key -out $(OUTPUT_DIR)/cert/server.csr \
		-subj "/C=CN/ST=Guangdong/L=Guangzhou/O=serverdevops/OU=serverit/CN=localhost/emailAddress=kalandramo@gmail.com" \
...
# 伪目标（防止文件与目标名称冲突）
.PHONY: gen.add-copyright gen.ca gen.protoc gen.generate gen.wire
