...
import (
	...
	"github.com/srxstack/srxstack/pkg/db"
	"gorm.io/gen"
	"gorm.io/gen/field"
	"gorm.io/gorm"
)
...
var generateConfigs = map[string]GenerateConfig{
	"gt": {ModelPackagePath: "./internal/apiserver/model", GenerateFunc: GenerateGinTplModels},
}

// 命令行参数.
var (
	addr       = pflag.StringP("addr", "a", "127.0.0.1:3306", "MySQL host address.")
	username   = pflag.StringP("username", "u", "gintpl", "Username to connect to the database.")
	password   = pflag.StringP("password", "p", "gintpl(#)888", "Password to use when connecting to the database.")
	database   = pflag.StringP("db", "d", "gintpl", "Database name to connect to.")
	modelPath  = pflag.String("model-pkg-path", "", "Generated model code's package name.")
	components = pflag.StringSlice("component", []string{"gt"}, "Generated model code's for specified component.")
...
// GenerateGinTplModels 为 GinTpl 组件生成模型.
func GenerateGinTplModels(g *gen.Generator) {
	g.GenerateModelAs(
		"user",
		"UserM",
