...
import (
	...
	"github.com/srxstack/gintpl/internal/apiserver/handler"
...
func (c *ServerConfig) InstallRESTAPI(engine *gin.Engine) {
	...
	v1 := engine.Group("/v1")
	{
		...
		// frostmournes 第三方服务示例路由
		frostmournev1 := v1.Group("/frostmournes")
		{
			frostmournev1.Use(authMiddlewares...)
			frostmourneUserV1 := frostmournev1.Group("/users")
			{
				frostmourneUserV1.POST("", handler.CreateFrostmourneUser)
			}
			frostmourneDepV1 := frostmournev1.Group("/deps")
			{
				frostmourneDepV1.POST("", handler.CreateFrostmourneDep)
			}
			frostmourneTeamV1 := frostmournev1.Group("/teams")
			{
				frostmourneTeamV1.POST("", handler.CreateFrostmourneTeam)
			}
		}
	}
}
